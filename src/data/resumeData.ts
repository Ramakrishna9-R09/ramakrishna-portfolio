...
export const resumeDb: ResumeDatabase = {
  ...
  projects: [
    {
      ...
      architecture: 'Event-driven serverless architecture using API Gateway for REST endpoints, Cognito for client JWT-based validation, S3 for storing document files, Lambda for execution (with Python cold start tunings), and prompt-engineered LLM engines for markdown output parser.',
      github: 'https://github.com/Ramakrishna9-R09/StudyMate'
    },
    {
      ...
      architecture: 'Embedded hardware pipeline connecting ESP32-CAM video stream over sockets to a local Linux host server running a concurrent Python classifier. Multi-threading is utilized to handle camera frames in thread A and execute TensorFlow/CNN classification on thread B without UI stutter.',
      github: 'https://github.com/Ramakrishna9-R09/IoT-Scalp-Disease'
    },
    {
      title: 'VIT Connect',
      ...
      architecture: 'Standard MERN (MongoDB, Express, React, Node) architecture. Focused heavily on database scalability. Handled complex feeds and community metrics through customized MongoDB aggregate queries and compound indexing on posts and user tags.',
      github: 'https://github.com/Ramakrishna9-R09/VIT-Connect'
    },
    {
      ...
      architecture: 'Type-safe React flow-editor utilizing React hooks and TypeScript schemas. Backend handles layout validations and schema mapping via Node Express server, storing workflow graphs in standard JSON trees.',
      github: 'https://github.com/Ramakrishna9-R09/HR-Workflow-Designer'
    }
  ],
  certifications: [
    {
  ...
  education: [
    {
      ...
      duration: '2022 -- 2027',
      metric: { label: 'CGPA', value: '8.27/10' },
      highlights: [
    ...
    },
    {
      ...
      duration: '2020 -- 2022',
      metric: { label: 'Board Score', value: '97%' },
      highlights: [
    ...
    },
    {
      ...
      duration: '2019 -- 2020',
      metric: { label: 'State Board Score', value: '98%' },
      highlights: [
...
// A lightweight Keyword search matching scoring helper (Mini Client RAG Search Index)
export function searchResumeIndex(query: string): { context: string[]; response: string } {
  ...
  if (!q) {
    return {
      context: ['Null Query.'],
      response: 'Please ask a valid query about Venkata Ramakrishna\'s resume.'
    };
  }

  // Keywords weights matches
  ...
  if (isEducation) {
    context.push(
      `Context 1: "Integrated M.Tech Software Engineering at ${resumeDb.education[0].institution}, ${resumeDb.education[0].metric.label} ${resumeDb.education[0].metric.value}."`,
      `Context 2: "Intermediate (XII) score ${resumeDb.education[1].metric.value} at Saraswathi Junior College | Class X score ${resumeDb.education[2].metric.value}."`
    );
    response = `Venkata Ramakrishna Kamepalli is currently pursuing an Integrated M.Tech in Software Engineering at Vellore Institute of Technology (VIT) Chennai, holding an active CGPA of 8.27/10. Previously, he achieved an Intermediate score of 97% and a Secondary School Board score of 98%.`;
  } else if (isAI) {
    context.push(
      `Context 1: "Certified in Claude API, MCP (Model Context Protocol), and Agent Skills by Anthropic (May 2026)."`,
      `Context 2: "StudyMate application delivers 5 LLM features (MCQ, Summarization, Flashcards, Mindmaps, Notes) in Python on AWS."`
    );
    response = `He holds official Anthropic certifications in 'Claude with the Anthropic API', 'Model Context Protocol (MCP)', and 'Introduction to Agent Skills' (May 2026). Practically, he applied these tools in 'StudyMate', coding Lambda pipelines using prompt engineering to deliver 5 AI-driven study features.`;
  } else if (isProjects) {
    context.push(
      `Context 1: "Projects: StudyMate (AWS Serverless AI), IoT Scalp Disease Detector (CNN Classifier), VIT Connect (Student Community), HR Workflow Designer."`,
      `Context 2: "StudyMate latency reduced 20%. IoT reliability increased 40% via multithreading. MERN DB query optimized by 20%."`
    );
    response = `Venkata Ramakrishna has built 4 key systems: (1) StudyMate, a serverless AI study platform; (2) IoT Scalp Disease Detection, an embedded C++/Python pipeline running multi-threaded CNNs; (3) VIT Connect, a MERN community platform; (4) HR Workflow Designer, a type-safe drag-and-drop React flow-builder.`;
  } else if (isSkills) {
    context.push(
      `Context 1: "Skills categories: GenAI, Python & OOP, AWS Serverless Cloud, NoSQL Databases (MongoDB Associate Developer)."`,
      `Context 2: "Ecosystem capabilities: Pytest testing, multi-threaded concurrency, clean code reviewing."`
    );
    response = `His technical stack is anchored in Python (OOP, Pytest validation, multi-threading) and AWS serverless systems (Lambda, S3, API Gateway, Cognito). He is a Certified MongoDB Associate Developer and is highly proficient in TypeScript, Node, and REST API architectures.`;
  } else if (isContact) {
    context.push(
      `Context 1: "Availability: ${resumeDb.contact.availability}."`,
      `Context 2: "Contact details: Email: ${resumeDb.contact.email} | Phone: ${resumeDb.contact.phone} | Location: ${resumeDb.contact.location}."`
    );
    response = `You can reach Venkata Ramakrishna Kamepalli via email at ${resumeDb.contact.email} or by phone at ${resumeDb.contact.phone}. He is located in Hyderabad, India, and is available for Full-time, Work-from-Office roles, open to relocations to Pune, Bangalore, Hyderabad, or Chennai.`;
  } else {
    context.push(
      `Context 1: "Principal Summary: Python-proficient software engineering student with M.Tech from VIT Chennai."`,
      `Context 2: "Certified in Claude API, MCP, Agent Skills, and MongoDB."`
    );
    response = `Venkata Ramakrishna Kamepalli is a Software Engineering student at VIT Chennai. He is an Anthropic Certified Agent Specialist and a MongoDB Certified Associate Developer, specializing in robust Python backends, concurrent pipelines, and AWS serverless AI architectures.`;
  }
